...
void Tree::enter(const std::vector<std::string>& tokens) {
    clear();
    ...
    root = buildTree(tokens, offset);

    if (printTreeAfterEnter) {
        printTreeAfterEnter = false;
        std::cout << "Za malo argumentow. Uzupelniono wartosciami 1.0" << std::endl;
        toString();
    }

    if (offset < tokens.size()) {
        std::cout << "Zignorowano nadmiarowe argumenty." << std::endl;
    }
}

Node* Tree::buildTree(const std::vector<std::string>& tokens, int& offset) {
    if (offset >= tokens.size()) {
        printTreeAfterEnter = true;
    ...
    while (newNode->getChildren().size() < newNode->numberOfArguments()) {
        Node* child = buildTree(tokens, offset);
